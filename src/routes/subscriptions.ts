...
router.get("/me", async (request, response) => {
  try {
    ...
    if (auth.payload.role === "SALESMAN") {
      ...
      if (shopUser?.shop) {
        const access = await evaluateShopSubscriptionAccess(shopUser.shop.id);
        const invoice = access.billingDate ? await ensureDailyInvoice(shopUser.shop.id) : null;
        return response.json({
          shop: {
            id: shopUser.shop.id,
            shopCode: shopUser.shop.shopCode,
            shopName: shopUser.shop.shopName,
