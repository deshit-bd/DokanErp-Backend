...
router.get("/stock", async (request, response) => {
  try {
    ...
    // Fetch all shop products to count status
    const shopProducts = await prisma.shopProduct.findMany({
      where: { shopId },
      include: {
        masterProduct: {
    ...
    const lowStockItems: Array<{ name: string; category: string; quantity: number; status: string }> = [];
    ...
    for (const p of shopProducts) {
      ...
      if (stock <= 0) {
        ...
        lowStockItems.push({
          name: p.masterProduct?.name || p.localName || "অজানা পণ্য",
          category: p.masterProduct?.category?.name || p.localCategory || "স্টক",
          quantity: Math.round(stock),
          status: "Out of Stock",
        });
      } else if (stock <= limit) {
    ...
    // Query stock movements in the range
    ...
    const shopProductIds = [...new Set(dbMovements.map((m: any) => m.shopProductId))];
    const movementProducts = shopProductIds.length > 0
      ? await prisma.shopProduct.findMany({
    ...
    const movementProductsMap = new Map(movementProducts.map((p) => [p.id, p]));

    let stockIn = 0;
    let stockOut = 0;

    const movements = dbMovements.map((m: any) => {
      const p = movementProductsMap.get(m.shopProductId);
      ...
      if (isPositive) {
        ...
        stockOut += Math.abs(delta);
      }

      return {
        name: p?.masterProduct?.name || p?.localName || "অজানা পণ্য",
        category: p?.masterProduct?.category?.name || p?.localCategory || "স্টক",
        quantity: Math.round(Math.abs(delta)),
        type: m.movementType || (isPositive ? "IN" : "OUT"),
    ...
    return response.status(503).json({ message: "Stock report could not be loaded." });
  }
});
