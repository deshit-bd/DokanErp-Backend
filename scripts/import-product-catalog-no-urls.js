...
const FILE_PATH = 'C:/Users/sajib/Downloads/product-catalog-FILLED/product-catalog-FILLED/no_urls_product-catalog-FILLED 1-7749.xlsx';
...
async function main() {
  ...
  let created = 0, updated = 0, skipped = 0, emojiUsed = 0;
  const skippedRows = [];
  ...
  for (const batch of batches) {
    await Promise.all(batch.map(async (r) => {
      ...
      const rawPictureUrl = r['Image URL'] != null ? String(r['Image URL']).trim() : '';
      const pictureUrl = rawPictureUrl.length > 0 ? rawPictureUrl : DEFAULT_PRODUCT_EMOJI;
      if (pictureUrl === DEFAULT_PRODUCT_EMOJI) emojiUsed++;
  ...
  console.log('--- DONE ---');
  console.log({ created, updated, skipped, emojiUsed });
  ...
  const finalCounts = {
    ...
    productsWithEmojiPicture: await prisma.masterProduct.count({ where: { pictureUrl: DEFAULT_PRODUCT_EMOJI } }),
  };
